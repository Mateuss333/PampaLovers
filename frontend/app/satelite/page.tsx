...
export default function SatelitePage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        setMLProgress(await getMLAnalysisProgress())
      } catch {
        setMLProgress({ status: "No disponible", progress: 0 })
      }
      setLoading(false)
    }
    loadData()
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Mapa base con tus lotes dibujados cuando tengan polígono guardado. El selector de capa y la escala NDVI son
            referencia; capas analíticas reales pueden sumarse después.
          </p>
        </div>
